...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        if not hasattr(templates, camel_case_name):
            continue
        templates_dict: dict[str, list[models.IsaTableFileTemplate]] = getattr(
            templates, camel_case_name
        )
        file_content = json.loads(file.read_text())
        for key, input_data in file_content.items():
            templates_dict[key] = []
            for x in input_data:
                templates_dict[key].append(
                    models.IsaTableFileTemplate.model_validate(x)
                )
