...
get_table_ontologies(meta, table_file, header) := result if {
	...
	values_set := {sprintf("[%v, %v, %v]", [x, y, z]) |
	...
	values := [x | some x in values_set]
	count(values) > 0

	values_str := concat(",", values)
	...
	result := {
		...
		"sourceFile": table_file.filePath,
...
get_table_unit_ontologies(meta, files) := result if {
	result := {{
		...
		some filename, table_file in files
