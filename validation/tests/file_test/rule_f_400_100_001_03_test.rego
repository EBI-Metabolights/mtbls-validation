#########################################################################################################
# Unit tests for rule_f_400_100_001_03
#########################################################################################################
...
rule_f_400_100_001_03_test_cases := 1
...
test_rule_f_400_100_001_03_no_violation_01 if {
	result := rules.rule_f_400_100_001_03 with input as {
		...
		"assays": {
			"a_MTBLS1.txt": {
				"assayTechnique": {"name": "NMR", "mainTechnique": "NMR"}, "table": {
...
test_rule_f_400_100_001_03_violation_01 if {
	...
	input_data := json.patch(
		...
		[
			{
				"op": "replace",
				"path": "/referencedDerivedFiles",
				"value": [],
			},
			{
				"op": "replace",
				"path": "/studyFolderMetadata/files",
				"value": {"x.mzML": {"extension": ".mzML"}},
			},
		],
	)

	result := rules.rule_f_400_100_001_03 with input as input_data
	count(result) == 1
}
